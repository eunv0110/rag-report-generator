#!/usr/bin/env python3
"""BM25 Retriever 성능 평가 with Langfuse + Ragas"""
...
def prepare_ragas_dataset(
    ...
    Args:
        ...
        contexts: 검색된 컨텍스트 리스트 (각 질문당 여러 개의 컨텍스트)
        answers: 생성된 답변 리스트
...
def evaluate_bm25_retrieval(
    ...
    """
    BM25 검색 성능을 Ragas로 평가하고 Langfuse에 로깅
    ...
    """
    print("=" * 60)
    print("�� BM25 Retrieval 평가 시작")
    print("=" * 60)

    # Langfuse 클라이언트 초기화
    langfuse = get_langfuse_client()

    # Qdrant 클라이언트 및 BM25 Retriever 초기화
    qdrant_client = QdrantClient(path=QDRANT_PATH)
    retriever = BM25Retriever(qdrant_client, use_korean_tokenizer=use_korean_tokenizer)
    ...
    ground_truths = []
    contexts_list = []
    answers = []  # BM25는 검색만 수행하므로 검색 결과를 답변으로 사용
    ...
    for item in eval_data:
        ...
        # 검색 결과의 첫 번째를 답변으로 사용 (또는 여러 개를 결합)
    ...
    # Ragas 평가 실행
    # Context-based metrics만 사용 (BM25는 검색만 수행하므로)
    metrics = [
        context_precision,
        context_recall,
    ]
        ...
        if langfuse:
            ...
            for metric_name, score in results.items():
                if isinstance(score, (int, float)):
                    langfuse.score(
                        name=f"bm25_{metric_name}",
                        value=score,
                        data_type="numeric",
                        comment=f"BM25 Retrieval - {metric_name}"
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BM25 검색 성능 평가")
    parser.add_argument(
        "--dataset",
        type=str,
